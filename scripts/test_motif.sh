...
if [ $1 == "0" ]; then
    ...
    export NUM_GPUS=1
    echo "Testing SGDet"
    MODEL_NAME="20221108074442_vctree_pairwise_sggen_4GPU_gcp_1e3"
    python  -u  -m torch.distributed.launch --master_port 10038 --nproc_per_node=$NUM_GPUS \
            tools/relation_test_net.py \
            --config-file "configs/e2e_relation_X_101_32_8_FPN_1x_motif.yaml" \
            MODEL.ROI_RELATION_HEAD.USE_GT_BOX False \
            MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL False \
            MODEL.ROI_RELATION_HEAD.PREDICTOR VCTree \
