...
sg_train_path = model_path+model_name+'/inference/sentence2graph/sen2graph.pytorch'
sg_test_path = model_path+model_name+'/inference_test/sentence2graph/sen2graph.pytorch'
output_path = model_path+model_name+'/sentence2graph/sentence_retrieval_model'+'_output_%s_%d.pytorch'
        ...
        for iteration, (fg_imgs, fg_txts, bg_imgs, bg_txts) in enumerate(train_data_loader):
            ...
            for fg_img, fg_txt, bg_img, bg_txt in zip(fg_imgs, fg_txts, bg_imgs, bg_txts):
                ...
                if fg_img['relations'].size(0) != 0 and fg_txt['relations'].size(0) != 0:
                    con_flag = 0
                if bg_img['relations'].size(0) != 0 and bg_txt['relations'].size(0) != 0:
                    con_flag = 0
            if con_flag == 1:
                continue
            ...
            # clip_grad_norm([(n, p) for n, p in model.named_parameters() if p.requires_grad], max_norm=cfg.SOLVER.GRAD_NORM_CLIP,
            #                logger=logger, verbose=verbose, clip=True)
            ...
            if iteration % 100 == 0:
                logger.info("epoch: {epoch} loss: {loss:.6f} lr: {lr:.6f}".format(epoch=epoch, loss=float(
