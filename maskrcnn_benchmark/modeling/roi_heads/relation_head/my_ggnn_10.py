...
from torch.nn import Module, Linear, ModuleList, Sequential, GELU, ReLU, LayerNorm, Flatten
...
class GGNN(Module):
    def __init__(self, emb_path, graph_path, time_step_num=3, hidden_dim=512, \
        ...
        if self.use_ontological_adjustment is True:
            ...
            # TODO: add a matrix here
            ontological_preds = self.adjmtx_pred2pred
            self.eoa_flatten = Flatten()
            self.eoa_compress = Linear(4*51, 51)
            self.eoa_activation = GELU()
            ...
            if self.normalize_eoa is True:
                F_normalize(self.ontological_preds, p=1, out=self.ontological_preds)
    ...
    def forward(self, rel_inds, obj_probs, obj_fmaps, vr):
        ...
        # what is rel_inds[: 0] vs :, 1?

        # edges_img_pred2subj = torch_zeros((num_img_pred, num_img_ent), dtype=torch_float32, device=CUDA_DEVICE, requires_grad=False)
        edges_img_pred2subj = [torch_zeros((num_img_pred, num_img_ent), dtype=torch_float32, device=CUDA_DEVICE, requires_grad=False) for num_image_pred, num_image_int in zip()]
        ...
        edges_img_pred2subj = []
        for num_img_ent, num_img_pred in zip():
            edges_img_pred2subj_i = torch_zeros((num_img_pred, num_img_ent), dtype=torch_float32, device=CUDA_DEVICE, requires_grad=False)
            edges_img_pred2subj_i[arange(num_img_pred), rel_inds[:, 0]] = 1
            edges_img_pred2subj.append(edges_img_pred2subj_i)
        ...
        for t in range(self.time_step_num):
            ...
            # NOTE: there's some vectorization opportunity right here, with matmul
            ...
            nodes_ont_ent = (1.0 - z_ont_ent) * nodes_ont_ent + z_ont_ent * h_ont_ent
            ...
            nodes_ont_pred = (1.0 - z_ont_pred) * nodes_ont_pred + z_ont_pred * h_ont_pred
            ...
            nodes_img_ent = (1.0 - z_img_ent) * nodes_img_ent + z_img_ent * h_img_ent
            ...
            nodes_img_pred = (1.0 - z_img_pred) * nodes_img_pred + z_img_pred * h_img_pred
            ...
            if self.use_ontological_adjustment is True and not self.merge_eoa_sa:
                # pred_cls_logits = (ontological_preds_prod @ pred_cls_logits.T).T
                # TODO: try to concat and compress with an additional fc instead
                pred_cls_logits = pred_cls_logits + self.eoa_activation(self.eoa_compress(self.eoa_flatten((self.ontological_preds @ pred_cls_logits.T).T)))
