...
@registry.ROI_RELATION_PREDICTOR.register("GSCPredictor")
class GSCPredictor(Module):
    def __init__(self, config, in_channels):
        super(GSCPredictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        # load parameters`
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
    ...
    def per_image(self, proposal, rel_pair_idx, roi_features_image, union_features_image, device=None, debug=True):
        ...
        for t in range(self.time_step_num):
            ...
            # message_received_ont_ent_new = mm_flatten_cat_transpose_first_inplace(edges_ont_ent2ent, message_send_ont_ent, edges_ont_pred2ent, message_send_ont_pred, edges_img2ont_ent, message_send_img_ent)
            # try:
            #     # assert torch_allclose(message_received_ont_ent, message_received_ont_ent_new, atol=0.1, rtol=0.2)
            #     assert torch_allclose(message_received_ont_ent, message_received_ont_ent_new, rtol=1e-03)
            #     # import torch
            #     # message_received_ont_ent[~torch.isclose(message_received_ont_ent, message_received_ont_ent_new, atol=0.1, rtol=0.2)]
            #     # message_received_ont_ent[~torch.isclose(message_received_ont_ent, message_received_ont_ent_new, rtol=1e-03)]
            #     # message_received_ont_ent_new[~torch.isclose(message_received_ont_ent, message_received_ont_ent_new, atol=0.1, rtol=0.2)]
            #     # message_received_ont_ent_new[~torch.isclose(message_received_ont_ent, message_received_ont_ent_new, rtol=1e-03)]
            # except:
            #     breakpoint()
            message_received_ont_ent = self.fc_mp_receive_ont_ent(message_received_ont_ent) # message_received_ont_ent: torch.Size([177, 3328]) => torch.Size([177, 1024])
            # message_received_ont_ent_new = self.fc_mp_receive_ont_ent(message_received_ont_ent_new)
            # breakpoint()
