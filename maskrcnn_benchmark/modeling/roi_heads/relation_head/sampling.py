...
from torch import int64 as torch_int64, cat as torch_cat, ones as torch_ones, eye as torch_eye, nonzero as torch_nonzero, zeros as torch_zeros, randperm as torch_randperm, as_tensor as torch_as_tensor
...
class RelationSampling(object):
    ...
    def prepare_test_pairs(self, device, proposals):
        ...
        for p in proposals:
            n = len(p)
            cand_matrix = torch_ones((n, n), device=device) - torch_eye(n, device=device)
            ...
            idxs = torch_nonzero(cand_matrix).view(-1,2)
            if len(idxs) > 0:
                rel_pair_idxs.append(idxs)
            else:
                # if there is no candidate pairs, give a placeholder of [[0, 0]]
                rel_pair_idxs.append(torch_zeros((1, 2), dtype=torch_int64, device=device))
        return rel_pair_idxs


    def gtbox_relsample(self, proposals, targets, times_bg=None):
        #times_bg=2
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            tgt_pair_idxs = torch_nonzero(tgt_rel_matrix > 0)
            ...
            # sym_binary_rels
            binary_rel = torch_zeros((num_prp, num_prp), device=device, dtype=torch_int64)
            binary_rel[tgt_head_idxs, tgt_tail_idxs] = 1
            binary_rel[tgt_tail_idxs, tgt_head_idxs] = 1
            rel_sym_binarys.append(binary_rel)

            rel_possibility = torch_ones((num_prp, num_prp), device=device, dtype=torch_int64) - torch_eye(num_prp, device=device, dtype=torch_int64)
            rel_possibility[tgt_head_idxs, tgt_tail_idxs] = 0
            tgt_bg_idxs = torch_nonzero(rel_possibility > 0)

            # generate fg bg rel_pairs
            if tgt_pair_idxs.shape[0] > num_pos_per_img:
                perm = torch_randperm(tgt_pair_idxs.shape[0], device=device)[:num_pos_per_img]
            ...
            if times_bg is not None:
                num_bg = min(num_bg, int(times_bg * num_fg))
                num_bg = min(num_bg, tgt_bg_idxs.shape[0])
            perm = torch_randperm(tgt_bg_idxs.shape[0], device=device)[:num_bg]
            tgt_bg_idxs = tgt_bg_idxs[perm]

            img_rel_idxs = torch_cat((tgt_pair_idxs, tgt_bg_idxs), dim=0)
            img_rel_labels = torch_cat((tgt_rel_labs.long(), torch_zeros(tgt_bg_idxs.shape[0], device=device, dtype=torch_int64)), dim=0).contiguous().view(-1)
    ...
    def motif_rel_fg_bg_sampling(self, device, tgt_rel_matrix, ious, is_match, rel_possibility):
        ...
        for i in range(num_tgt_rels):
            # generate binary prp mask
            bi_match_head = torch_nonzero(binary_prp_head[i] > 0)
            bi_match_tail = torch_nonzero(binary_prp_tail[i] > 0)
            ...
            prp_head_idxs = torch_nonzero(is_match[tgt_head_idx]).squeeze(1)
            prp_tail_idxs = torch_nonzero(is_match[tgt_tail_idx]).squeeze(1)
            ...
            fg_labels = torch_as_tensor([tgt_rel_lab]*prp_tail_idxs.shape[0], dtype=torch_int64, device=device).view(-1,1)
            fg_rel_i = cat((prp_head_idxs.view(-1,1), prp_tail_idxs.view(-1,1), fg_labels), dim=-1).to(torch_int64)
            ...
            if fg_rel_i.shape[0] > self.num_sample_per_gt_rel:
                ...
                perm = np_random_choice(ious_score.shape[0], p=ious_score, size=self.num_sample_per_gt_rel, replace=False)
        ...
        if len(fg_rel_triplets) == 0:
            fg_rel_triplets = torch_zeros((0, 3), dtype=torch_int64, device=device)
        else:
            fg_rel_triplets = cat(fg_rel_triplets, dim=0).to(torch_int64)
            if fg_rel_triplets.shape[0] > self.num_pos_per_img:
                perm = torch_randperm(fg_rel_triplets.shape[0], device=device)[:self.num_pos_per_img]
                fg_rel_triplets = fg_rel_triplets[perm]

        # select bg relations
        bg_rel_inds = torch_nonzero(rel_possibility>0).view(-1,2)
        bg_rel_labs = torch_zeros(bg_rel_inds.shape[0], dtype=torch_int64, device=device)
        bg_rel_triplets = cat((bg_rel_inds, bg_rel_labs.view(-1,1)), dim=-1).to(torch_int64)
        ...
        if bg_rel_triplets.shape[0] > 0:
            perm = torch_randperm(bg_rel_triplets.shape[0], device=device)[:num_neg_per_img]
            bg_rel_triplets = bg_rel_triplets[perm]
        else:
            bg_rel_triplets = torch_zeros((0, 3), dtype=torch_int64, device=device)

        # if both fg and bg is none
        if fg_rel_triplets.shape[0] == 0 and bg_rel_triplets.shape[0] == 0:
            bg_rel_triplets = torch_zeros((1, 3), dtype=torch_int64, device=device)
