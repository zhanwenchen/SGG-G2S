...
class GSCMPContext(Module):
    ...
    def forward(self, roi_features, proposals, union_features, rel_pair_idxs, logger=None, global_image_features=None):
        num_objs = [len(b) for b in proposals]

        obj_rep = self.obj_unary(roi_features)
        rel_rep = F.relu(self.edge_unary(union_features))
        gsc_rep = self.gsc_unary(roi_features)

        obj_count = obj_rep.shape[0]
        rel_count = rel_rep.shape[0]
        gsc_count = gsc_rep.shape[0]
