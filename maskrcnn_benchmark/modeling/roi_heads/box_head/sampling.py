...
from torch import nonzero as torch_nonzero, int64 as torch_int64
...
class FastRCNNSampling(object):
    ...
    def prepare_targets(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            labels_per_image = labels_per_image.to(dtype=torch_int64, non_blocking=True)

            # Label background (below the low threshold)
            bg_inds = matched_idxs_per_image == BELOW_LOW_THRESHOLD
            labels_per_image[bg_inds] = 0

            # Label ignore proposals (between low and high thresholds)
            ignore_inds = matched_idxs_per_image == BETWEEN_THRESHOLDS
    ...
    def assign_label_to_proposals(self, proposals, targets):
        for img_idx, (target, proposal) in enumerate(zip(targets, proposals)):
            ...
            labels_per_image = matched_targets.get_field("labels").to(dtype=torch_int64, non_blocking=True)
