...
detected_path = model_path+model_name+'/inference/VG_stanford_filtered_with_attribute_test/'
# where to save the generated annotation
output_path = model_path+model_name+'/inference/sentence2graph/sen2graph_wobg.pytorch'
...
def generate_txt_img_sg(img_sg, txt_sg):
    ...
    for i in range(num_img):
        ...
        if cap_graph['vg_valids'][i] and (coco_id in img_sg) and (coco_id in txt_sg):
            ...
            txt_img_sg[coco_id] = {'img':encode_img, 'txt':encode_txt,'image_graph':image_graph,'text_graph':text_graph}
    return txt_img_sg
...
torch.save(txt_img_sg,output_path)
